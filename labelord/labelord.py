...
# File: labelord.py
"""from .web import run_server, app
from .cli import list_labels, list_repos, run, cli
...
def actualize(ctx, repo, template, verbose, dry_run, quiet, replace):
    ...
    for template_label in template:
        ...
        if ((not is_included(target_labels, template_label) and is_named(target_labels, template_label)) or
            ...
            if (dry_run == False):
                p = session.patch("https://api.github.com/repos/" + repo + "/labels/" + template_label.n.lower(),
                                  json_data)
            message = '[UPD]'
            if (dry_run):
                message += '[DRY] '
...
@click.option('--config', '-c', default='labelord/config.cfg',
...
def run(ctx, mode, template_repo, verbose, dry_run, quiet, all_repos):
    ...
    if (real_template_repo != None):
        while True:
            r = session.get(
                'https://api.github.com/repos/' + str(real_template_repo) + "/labels?per_page=100&page=" + str(page))
            if (r.status_code == 200):
                for i in r.json():
                    color = i['color']
                    name = i['name']
